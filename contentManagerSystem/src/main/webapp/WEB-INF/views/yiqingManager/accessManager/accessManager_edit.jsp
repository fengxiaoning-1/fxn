	...
	<div class="layui-form-item">
		<div class="layui-inline">
			...
			<div class="layui-input-inline">
				<input type="text" class="layui-input" id="DoorName" name="DoorName" lay-verify="required|name" maxlength="20" value="${department.doorName}" placeholder="请输入岗亭名称">
			</div>
		</div>
		<div class="layui-inline">
			<label class="layui-form-label">所属工厂</label>
			<div class="layui-input-inline">
				<select id="parkId" name="parkId" lay-filter="factoryFilter" lay-verify="required">
					<c:forEach var="fac" items="${fList}">
						<option value="${fac.id}"
								<c:if test="${fac.id==department.parkId}">
									selected</c:if>>${fac.name}
						</option>
					</c:forEach>
...
<script type="text/javascript">
    layui.config({
        ...
        form.on("submit(saveDepartment)",function(data){
            ...
            //登陆验证
            $.ajax({
                url : '${ctx}/passDoor/passDoor_updateSave.do',
                type : 'post',
                async: false,
                data : data.field,
