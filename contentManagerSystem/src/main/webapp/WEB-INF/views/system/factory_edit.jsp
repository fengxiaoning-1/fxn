    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <c:if test="${pageFlag == 'addPage' }">
             <input type="text" class="layui-input" name="name" lay-verify="required|name" maxlength="20" value="" placeholder="请输入工厂名称">
            </c:if>
            <c:if test="${pageFlag == 'updatePage' }">
                <input type="text" class="layui-input" name="name" lay-verify="required|name" maxlength="20"  value="${factory.name}" placeholder="请输入工厂名称" >
            </c:if>
        </div>
    </div>
...
<script type="text/javascript">
    layui.config({
        ...
        form.on("submit(saveUser)",function(data){
            ...
            //登陆验证
            $.ajax({
                url : '${ctx}/factory/ajax_save_factory.do',
                type : 'post',
                async: false,
                data : data.field,
